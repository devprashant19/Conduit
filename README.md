...
Conduit is a web-based control center for coding CLI agents (Claude Code, Codex, Gemini, OpenCode). While autonomous agent platforms hand the steering wheel to AI, Conduit keeps you in the driver's seat: see every agent's screen in one window, coordinate them manually, and intervene the moment something goes sideways.
...
Conduit is the opposite bet. You run 2–7 agents in parallel, each doing real work, but **you stay in the loop on every one**. No hidden decisions, no runaway loops, no "come back tomorrow and hope it went well."
...
Conduit gives you a browser-based dashboard, a per-project wiki, shared content folders, and native agent-to-agent messaging — all while you stay in control of every prompt.
...
```bash
git clone https://github.com/0x0funky/Conduit.git
cd Conduit
npm install
npm run dev
```
...
All agents can read/write shared files, and the Conduit web UI reflects changes in real-time via file watching.
...
1. When an agent starts, Conduit registers a session-scoped MCP server exposing two tools: `message_agent(target, message)` and `list_teammates()`.
...
4. Conduit writes the message into the target agent's PTY as `[Message from Frontend]: API is done`, so the target's LLM sees it as fresh user input.
...
- **Session-scoped, no global pollution** — Claude agents get a per-agent MCP config file at `~/.conduit/mcp-configs/<agentId>.json`. Your personal MCP setup in `~/.claude.json` is never modified.
