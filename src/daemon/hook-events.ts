 ...
 * every lifecycle event it receives. The Conduit dispatch layer (conduit.ts) listens
 * so it can tell when an `ask_agent` turn has started and finished.
 *
 * Kept in its own module so daemon.ts (emitter) and conduit.ts (listener) share
