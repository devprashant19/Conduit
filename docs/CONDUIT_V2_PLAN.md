...
> **Conduit v2 — JARVIS for your coding agent team.**
...
| **Conduit (v1)** | multi | **manual** | **PTY tied to web server** | — |

### Three findings

1. **The "central brain" concept has precedent — but Conduit's exact niche is open.**
...
2. **The delegation mechanism is mature.** LangGraph / CrewAI / AutoGen all use
   ...
   decides who to route to, collects results, and synthesizes. Conduit's
...
Conduit injects the input programmatically, so it stays subscription-covered.
...
└────────────────────┬─────────────────────────────────┘
                     │ local socket / WS
┌────────────────────▼─────────────────────────────────┐
│  conduit-daemon  ★ v2 core — long-lived process       │
│                                                       │
│  ┌─────────────────┐ ┌──────────────┐ ┌───────────┐ │
...
|--|--------------------------|--------------------------|
| Runtime | `codex app-server` — persistent, multi-thread, structured | PTY-hosted interactive `claude`; Conduit injects prompts, reads replies via hooks/JSONL |
...
- **Claude brain runs in a daemon-owned PTY** (interactive mode) so it stays
  subscription-covered. Conduit drives it by injecting prompts and detecting
...
|---------|-------|--------|
| **v2.0 Daemonize** | Standalone `conduit-daemon` owns PTYs; web is a client | ✅ done + tested |
...
|----------------|--------|-----|
| PTY interactive terminals (manual operation) | PTY moves from Express into the daemon | `conduit-daemon` process |
